...
import {
  ...
  x,
  y
} from "./chunk-UL3ORRLV.js";
import {
  Q,
  _
} from "./chunk-BQMT4RHU.js";
import {
  __commonJS,
  __export,
  __toCommonJS,
...
var SWRConfigContext = Q({});
var SWRConfig = (props) => {
  const { value } = props;
  const parentConfig = x(SWRConfigContext);
  const isFunctionalConfig = isFunction(value);
  const config = T(() => isFunctionalConfig ? value(parentConfig) : value, [
    isFunctionalConfig,
    parentConfig,
    value
  ]);
  const extendedConfig = T(() => isFunctionalConfig ? config : mergeConfigs(parentConfig, config), [
    isFunctionalConfig,
    parentConfig,
    config
  ]);
  const provider = config && config.provider;
  const cacheContextRef = A(UNDEFINED);
...
var useSWRConfig = () => {
  const parentConfig = x(SWRConfigContext);
  const mergedConfig = T(() => mergeObjects(defaultConfig, parentConfig), [
    parentConfig
  ]);
  return mergedConfig;
};
...
var use2 = Rn.use || // This extra generic is to avoid TypeScript mixing up the generic and JSX sytax
...
var useSWRHandler = (_key, fetcher, config) => {
  ...
  const initialMountedRef = A(false);
  const unmountedRef = A(false);
  const keyRef = A(key);
  const fetcherRef = A(fetcher);
  const configRef = A(config);
  const getConfig = () => configRef.current;
  const isActive = () => getConfig().isVisible() && getConfig().isOnline();
  const [getCache, setCache, subscribeCache, getInitialCache] = createCacheHelper(cache2, key);
  const stateDependencies = A({}).current;
  ...
  const cached = (0, import_shim.useSyncExternalStore)(q(
